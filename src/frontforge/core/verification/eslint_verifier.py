"""Runs eslint against the generated project (codegen stage only)."""
    ...
    async def verify(
        ...
        if not (session.generated_dir / "node_modules").exists():
            return [
                VerificationIssue(
                    verifier=self.name,
                    message="skipped: run `npm install` in the generated project to enable linting",
                    severity="warning",
                )
            ]
        result = await self.terminal.run(["npx", "eslint", "."], cwd=session.generated_dir)
        if result.ok:
            return []
